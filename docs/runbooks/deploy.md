...
The organization chooser reads current, token-scoped iamnim memberships on every request. Creating for another organization navigates to its dedicated Guidance `/new` route; SSO preserves that route and preselects the organization as entity/reference. Each target organization must have its own deployed instance and data directory. Membership in another organization never authorizes access to this instance.
