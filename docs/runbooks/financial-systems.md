...
Revenue is a separate designation from cashflow and accounting. Shopify provides orders/refunds for revenue; a recorded Shopify grant does not prove fresh order collection or automatically import revenue into a forecast. The Natural Beauty Club revenue designation is user-confirmed Shopify, recorded through the existing shared configuration API rather than a Guidance-owned setting. No cashflow/accounting choices are guessed.
